...
def draw_chart(ticker, period, title, unit, current_price=None, target_min=None, target_max=None, target_buy=None):
    try:
        ...
        fig.update_layout(title=f"{title} ({unit})", height=450, template="plotly_dark", xaxis_rangeslider_visible=False)
...
if df_sheet is not None:
    ...
    if not filtered_df.empty:
        ...
        st.title(f"🚀 {selected} ({dart_code}) 기업 가치")
        ...
        with tab2:
            ...
            if market_choice == "한국(KRW)":
                ...
                if display_df is not None:
                    ...
                    # 상단 지표 영역 (수정됨: 중복 숫자 제거)
                    c1, c2, c3 = st.columns(3)
                    c1.metric("10년 평균 EPS", f"{m10:,.0f}원")
                    c2.metric("5년 평균 EPS", f"{m5:,.0f}원")
                    c3.metric("최신 EPS 성장률 (vs 10년평균)", "", delta=f"{vs10y:+.1f}%")
                    
                    # 하단 지표 영역 (수정됨: 명칭 변경 및 중복 제거)
                    c4, c5, c6 = st.columns(3)
                    c4.metric(f"{len(raw_data)}년 연평균 성장 (CAGR)", f"{get_cagr(raw_data):+.1f}%")
                    c5.metric("최근 5년 연평균 성장", f"{get_cagr(raw_data.tail(5)):+.1f}%")
                    c6.metric("성장 모멘텀 (5년평균 vs 10년평균)", "", delta=f"{mavg:+.1f}%")
                    ...
                    fig.add_hline(y=m10, line_dash="dash", line_color="#FFAB00", secondary_y=True, annotation_text="10년평균")
                    fig.add_hline(y=m5, line_dash="dot", line_color="#D500F9", secondary_y=True, annotation_text="5년평균")
