...
def draw_chart(ticker, period, title, unit, target_min=None, target_max=None, target_buy=None, current_price=None):
    try:
        ...
        if current_price and current_price > 0:
            ...
            fig.add_annotation(
                xref="paper", x=0.5, y=current_price, 
                text=f"<b>현재가 {unit}{current_price:,.0f}</b>", 
                showarrow=False, xanchor="center", yshift=10,           
                font=dict(color="white", size=16),   
                bgcolor="#FF4081", bordercolor="white", borderwidth=1, opacity=0.9
            )
        ...
        if target_buy and target_buy > 0:
            ...
            fig.add_annotation(
                ...
                bgcolor="#FFFFFF", bordercolor="gray", borderwidth=1, opacity=0.9
            )
    ...
    if not filtered_df.empty:
        ...
        try:
            ...
            if current_p > 0:
                ...
                # 7년 연복리(CAGR) 계산
                cagr_min = ((t_min / current_p) ** (1/7) - 1) * 100 if t_min > 0 else 0
                cagr_max = ((t_max / current_p) ** (1/7) - 1) * 100 if t_max > 0 else 0
            else:
                gap_min, gap_max, gap_buy = 0, 0, 0
                cagr_min, cagr_max = 0, 0
        except:
            current_p, t_min, t_max, t_buy = 0, 0, 0, 0
            gap_min, gap_max, gap_buy, cagr_min, cagr_max = 0, 0, 0, 0, 0
        ...
        with c3: 
            ...
            if cagr_min != 0:
                st.markdown(f"""<div style="background-color: #7B1FA2; padding: 5px 10px; border-radius: 5px; color: white; font-weight: bold; text-align: center; display: inline-block; margin-top: -15px; font-size: 0.9em;">📈 7~10년 연평균수익률 {cagr_min:+.1f}%</div>""", unsafe_allow_html=True)
        
        # 4. 최대 미래가치 + CAGR
        with c4: 
            st.metric("🚀 최대 미래가치 (목표)", f"{unit}{p_format.format(t_max)}", f"{gap_max:.1f}%")
            if cagr_max != 0:
                st.markdown(f"""<div style="background-color: #7B1FA2; padding: 5px 10px; border-radius: 5px; color: white; font-weight: bold; text-align: center; display: inline-block; margin-top: -15px; font-size: 0.9em;">📈 7~10년 연평균수익률 {cagr_max:+.1f}%</div>""", unsafe_allow_html=True)
